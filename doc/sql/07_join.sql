...
from
    ...
    inner join customers c
		on o.customer_id = c.customer_id; # 가져오는 데이터(join) 조건
# on과 where의 차이
# on은 join조건으로 두 테이블을 합쳐서 새로운 가상테이블을 만드는 것
...
# 왼쪽테이블 데이터는 다 출력되고, B는 매칭되는 것만 출
...
select # customer left join orders -> 전체 고객기준으로 orders 데이터를 붙히겠다.
	c.customer_id,
	c.customer_name,
    o.order_id,
    o.order_date
...
# 실습) 모든 주문에 대해 (orders테이블 시작으로) - inner join
...
select
    ...
    od.quantity
from
	orders o # orders에 product_id, customer_id -> join해야겠다!
inner join customers c
	on o.customer_id = c.customer_id
inner join order_details od
	on o.order_id = od.order_id
...
from
    ...
    left join orders o
    on c.customer_id = o.customer_id
where # order_id가 null -> 주문을 안했다. (주문안한 고객 리스트를 출력)
	o.order_id is null;
...
# 상품별(from product) 주문(order_details) 수 출력
# product left join order_details -> 주문이 0번인 상품도 함께 보여야 하기 때문 
select
	p.product_id,
    p.product_name,
    count(od.order_id) as `order_count` # order_details 한 row가 한번의 주문횟수를 의미
...
# 주문이 한번도 없는 상품을 조회(join)
# from product로 시작
select
	*
from
	product p
left join order_details od # od의 row 1개는 주문 1건에 해당
	on p.product_id = od.product_id # join결과 오른쪽 테이블이 null 이라는것 -> 주문 0건
    where
	order_detail_id is null; # null 과 비교는 반드시 is null/ is not null
    
# 가장 많이 팔린 상품(1등)을 조회하는 쿼리를 작성해 주세요
# from product, 팔린갯수(order_details.quantity)
# inner join -> product inner join order_details
# order_details inner join product 동일
...
# 서버에 주석으로  SET NULL을 문서화하는게 중요하다.
