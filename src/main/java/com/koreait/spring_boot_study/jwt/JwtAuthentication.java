...
// filter에서 검증이 성공하면 Authentication 객체를 만들고
...
public class JwtAuthentication implements Authentication {
    ...
    private final List<GrantedAuthority> authorities; // 유저의 권한(권한이 여러개일수도있어 List)
    private boolean isAuthenticated = true; // 인증여부

    @Override
    public Collection<? extends GrantedAuthority> getAuthorities() {
        return authorities;
        // ROLE_ADMIN or ROLE_USER 가 담겨있는걸 get해간다.
    ...
    public boolean isAuthenticated() {
        ...
        // true여야 SecurityContext에 저장된 Authenticated 객체기
        // 인증에 성공한다.
    }
    ...
    public String getName() {
        return userId; // 인증에 대한 식별자 - 사실상 사용자 식별과 동일
    }
}
