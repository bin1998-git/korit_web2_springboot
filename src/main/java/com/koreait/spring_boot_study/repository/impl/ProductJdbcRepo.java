...
public class ProductJdbcRepo implements ProductRepo {
    ...
    // conn, ps, rs -> AutoCloseable을 이식받고 있음
    ...
    public List<Product> findAllProducts() {
        ...
        ResultSet rs = null;
        // DB에서 올라오는 에러를 try-catch로 잡아줘야 한다.
        ...
        try {
            ...
            // prepareStatement에는 실제로 문자열로 sql쿼리가 들어가야 한다
            ps = conn.prepareStatement(sql);
            // 작성한 ps를 db에 전달하고, 실행시킨 결과를 가져온다.
            // DB에서 조회한 결과를 rs안에 테이블 형태로 들고온다고 보면 된다.
            rs = ps.executeQuery(); // select -> executeQuery(): rs를 리턴
            // rs.next는 테이블에서 한줄씩 읽어올건데, 그다음 줄이 존재하는지 검사하는 메서드
            while (rs.next()) {
                // next가 true이면 해당 줄의 컬럼 값들을 가져올 수 있음
                // product_id 컬럼값을 읽어 오세요
                int id = rs.getInt("product_id");
                // prodcut_name 컬럼값을 읽어 오세요
    ...
    public String findProductNameById(int id) {
        ...
        try{
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                return rs.getString("prduct_name");
            }
    ...
    public int updateProduct(int id, String name, int price) {
        ...
        try {
            ...
            ps.setInt(2,price);
            ps.setInt(3,id);
            return ps.executeUpdate(); // 단건이라 1 리턴될 것/ id가 이상하면 0리턴
        }catch (SQLException e) {
            e.printStackTrace();
        } finally {
            close(ps);
