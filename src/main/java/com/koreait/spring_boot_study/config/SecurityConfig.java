...
public class SecurityConfig {
    ...
    - CORS 에러를 방지하기 위해 설정
    브라우저(크롬, 웨일..) 에서 다른출처(origin)로 요청을 보낼 떄,
    브라우저정책 때문에 요청을 막아서 발생하는 에러
    -> origin이 다르면 에러가 발생
    *origin) 프로토콜(http, https), 도메인(localhost, naver.com), 포트(3000, 8080)
    위 3가지 요소가 모두 일치해야 같은 origin

    정통웹(SSR) 방식에서는 http:localhost:8080/page.html -> 사용자화면
    http:localhost:8080/main.js 주세요 요청가능
    http:localhost:8080/img.png 주세요 요청가능
    http:localhost:5173(브라우저 화면) 에서 localhost:8080으로 요청은 불가능(origin이 다름)
    -> 이때 브라우저는 이 상황을 '보안상 위험' 하다고 판단 -> CORS에러
