...
public class StudyRestController1 {
    ...
    // 클라이언트(브라우저) -> 서버로 전달
    // localhost:8080/study/test2?name="홍길동" -> study/test2 경로
    // http://서버주소/경로?name="홍길동"
    @GetMapping("/test2")
    public String test2(@RequestParam("name") String str) { //name에 str변수를 담은거
    ...
    // 자바의 Map과 유사하게 생겼음
    // 자바의 객체도 전송이 가능하다 -> (자바객체 -> JSON -> JavaScript객체)
    @GetMapping("/test5")
