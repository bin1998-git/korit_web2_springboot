...
public class StudyRestController3 {
    ...
    GET 요청을 제외한 모든 요청메서드에는 body가 존재한다.(보안이 관련된거는 거의 GET요청을 하지않는다)
    ...
    public String test1(@RequestBody Map<String, Object> data) {
        ...
        // 참고) Jackson라이브러리가 개입해서 자동으로 변환해준다.
        log.info("test1 컨트롤러 수신했습니다.");
    ...
    public String test2(@RequestBody StudyReqDto dto) {
        ...
        log.info("test2 컨트롤러 수신했습니다.");
        log.info("들어온 데이터: {}", dto);

        return "성공";
    }

    // 실습) test3라는 이름으로 AddPostReqDto 타입으로 데이터를 수신해주세요
    @PostMapping("/test3")
    public ResponseEntity<?> test3(@RequestBody AddPostReqDto add){
        // ResponseEntity: HTTP 응답을 자바에서 커스터마이징하기 편하게 만든 클래스
        // 제너릭타입을 받는다 -> body에 들어가는 데이터타입
        // HTTP 상태코드, body, header등을 쉽게 지정할 수 있다.
        log.info("test3 컨트롤러를 수신했습니다");
        log.info("들어온 데이터: {}", add);
